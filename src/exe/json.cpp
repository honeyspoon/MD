...
json to_json(const accepted_message_t& m) {
    return json{
        ...
        {"order_token", std::string(reinterpret_cast<const char*>(m.order_token), 14)},
...
json to_json(const replaced_message_t& m) {
    return json{
        ...
        {"cross_type", m.cross_type},
        {"order_state", m.order_state},
        {"previous_order_token", std::string(reinterpret_cast<const char*>(m.previous_order_token), 14)}
    };
}
...
void handler(const unsigned int, ouch::msg_header_t *msg_header) {
  ...
  case msg_type_t::SYSTEM_EVENT:
      {
        const auto* system_event = reinterpret_cast<const ouch::system_event_message_t*>(msg_header);
        std::cout << to_json(*system_event).dump() << std::endl;
        break;
      }
  case msg_type_t::ACCEPTED:
      {
        const auto* accepted = reinterpret_cast<const ouch::accepted_message_t*>(msg_header);
        std::cout << to_json(*accepted).dump() << std::endl;
        break;
      }
  case msg_type_t::EXECUTED: 
      {
        const auto* executed = reinterpret_cast<const ouch::executed_message_t*>(msg_header);
        std::cout << to_json(*executed).dump() << std::endl;
        break;
      }
  case msg_type_t::REPLACED:
      {
        const auto* replaced = reinterpret_cast<const ouch::replaced_message_t*>(msg_header);
        std::cout << to_json(*replaced).dump() << std::endl;
        break;
      }
  case msg_type_t::CANCELED:
      {
        const auto* cancelled = reinterpret_cast<const ouch::canceled_message_t*>(msg_header);
